...
Foi possível constatar alguns fatores durante a simulação:
\begin{itemize}
    \item \textbf{Little} \par
        Little pode ser observado durante toda a simulação.
    \item \textbf{Variância} \par
        Como esperado, a variância da fila LCFS se mostrou maior 
        que a da fila FCFS, quanto maior a utilização maior 
        a diferença entre as duas variâncias.
    \item \textbf{Kmin para LCFS} \par
        A fila LCFS precisou de valores maiores de Kmin
        para se estabilizar,
        e possuir intervalos de confiança válidos que contêm o valor 
        analítico calculado.
\end{itemize}
